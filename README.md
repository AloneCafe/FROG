...
## 🌻 效果
![ScreenShot](https://s3.bmp.ovh/imgs/2021/08/5b3ec9d9bb52ff8c.jpeg)
...
FROG 语言目前保留了 25 个关键字，分别是 

```func```、```break```、```continue```、```return```、```do```、
```while```、```if```、```else```、```for```、```switch```、```case```、```default```、```boolean```、
```byte```、```char```、```double```、```float```、```int```、```long```、```short```、```true```、
```false```、```len```、```asm```、```void```

> 关键字不能用于其他的用途，比如标识符的定义之类的操作。
...
目前版本的 FROG 编程语言支持用户定义一个不带参数的、返回值为 ```int``` （虽然定义上并不要求返回值） 并且名为 ```main``` 的函数，在从编译器生成的 IL 层面，汇编器将默认使用该函数作为虚拟机执行期间的入口点。如果不定义入口点，那么汇编器将不会生成可执行的字节码文件，虚拟机将无法执行该程序。
...
### 浅拷贝
在 FROG 编程语言中，矢量的互相赋值，实际传递的数据只是矢量句柄 (指向矢量本体的地址)，所以如果试图将矢量通过 ```=``` 运算符赋值给另外一个矢量，那么它们将指向同一个实际的矢量本体，或者说它们共享同一内存地址。
因此，矢量的赋值实际上是一种浅拷贝。为了避免这种情况发生，通常需要编写者手工完成深拷贝的操作。
...
相对应地，VRAM 是程序中用于存放矢量的空间，而矢量一般指 FROG 源程序层面定义的数组 (未来可能会扩充)。除了句柄 (或者说指向数组的指针) 以标量 (通常是 4 Byte 整数) 的形式放在 SRAM 中，数组本体是存放于 VRAM 中的。
...
字节码文件的布局排版采用稀疏格式，不强制要求区块的连续性。

| 文件偏移 | 字段大小 | 具体作用   |
| --------| ---- | ----- |
| +0 | 3 | 字符魔数标记：'F'、'V'、'M' |
| +3 | 1 | 0x0：不可被执行，0x1：可被执行 |
| +4 | 4 | 虚拟机执行入口点（基于函数代码块） |
| +8 | 4 | 静态代码块开始位置的偏移（基于文件） |
| +12 | 4 | 静态代码块的总大小 |
| +16 | 4 | 函数代码块开始位置的偏移（基于文件） |
| +20 | 4 | 函数代码块的总大小 |
| +24 | 不限定 | 总体的字节码数据 |

虚拟机加载字节码文件后，通过静态代码块开始位置的偏移与静态代码块的大小，读出整块静态代码块。同样地，虚拟机将通过函数代码块开始位置的偏移与函数代码块的大小，读出整块函数代码块。将两者装入 OPROM 之后，从静态代码块的开头开始，先执行完静态代码块；接着，从执行入口点的偏移开始（基于函数代码块开头的偏移，一般是用户定义的 main 函数位置）执行函数代码块，就这样一直执行下去直到程序退出。
