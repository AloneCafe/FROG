...
void AstBasicParser::parseLowLvTypeList_(TokenIter & it, std::vector<LocatedUtfString> & refLowLvTypeList) {
    for (;;) {
        if (it->isPunc<','>()) {
            ++it;
...
void AstBasicParser::parseLowLvTypeList(TokenIter & it, std::vector<LocatedUtfString> & refLowLvTypeList) {
    LocatedUtfString lowLvObjType;
    parseLowLvType(it, lowLvObjType);
    refLowLvTypeList.push_back(lowLvObjType);
 ...
 *            | scalar-type
 */
void AstBasicParser::parseLowLvType(TokenIter & it, LocatedUtfString & lowLvType) {
    AST_REC_LOCATION1
    /*if (it->isId()) {
...
void AstBasicParser::parseNoNumIndexList(TokenIter & it, int & degree) {
    if (it->isPunc<'['>()) {
        ++it;
        if (it->isPunc<']'>()) {
            ++it;
 ...
 */
void AstBasicParser::parseDefType(TokenIter & it, VarType & type) {
 ...
 *              | e
 */
void AstBasicParser::_parseFormalArgList1(TokenIter & it, std::vector<FormalArg> & args) {
    if (it->isPunc<','>()) {
        ++it;
        if (it->isEnd()) {
            AST_E(E_UNEXPECTED_EOF);
...
void AstBasicParser::_parseFormalArgList(TokenIter & it, std::vector<FormalArg> & args) {
    assert(it->isPunc<'('>());
    ++it;
 ...
 *              | e
 */
void AstBasicParser::_parseRealArgList1(TokenIter & it, std::vector<ExprPtr> & args) {
    if (it->isPunc<','>()) {
        ++it;
        if (it->isEnd()) {
            AST_E(E_UNEXPECTED_EOF);
...
void AstBasicParser::_parseRealArgList(TokenIter & it, std::vector<ExprPtr> & args) {
    assert(it->isPunc<'('>());
    ++it;
 ...
 */
/*
ExprPtr AstBasicParser::_buildExprDotList(TokenIter & it) {
    assert(it->isId());
    AST_REC_LOCATION1
 ...
 */
ExprPtr AstBasicParser::_buildExprIndexList(TokenIter & it, ExprPtr pLeft) {
    assert(it->isPunc<'['>());
    AST_REC_LOCATION1
    ++it;
 ...
 *
 */
ExprPtr AstBasicParser::_buildExprSuffix(TokenIter & it) {
    AST_REC_LOCATION1
    if (it->isPunc<'@'>()) {
        ++it;
        VarType objType;
 ...
 *            | e
 */
ExprPtr AstBasicParser::_buildExprUnary(TokenIter & it) {
 ...
 *               | ExprUnary
 */
ExprPtr AstBasicParser::_buildExprUnaryR2L(TokenIter & it) {
    ExprOp op;
    AST_REC_LOCATION1
    if (it->isPunc<'++'>()) {
 ...
 *                 | e
 */
ExprPtr AstBasicParser::_buildExprMulDivMod1(TokenIter & it, ExprPtr pLeft) {
    ExprOp op;
    AST_REC_LOCATION1
    if (it->isPunc<'*'>()) {
        op = ExprOp::OPT_MUL;
...
ExprPtr AstBasicParser::_buildExprMulDivMod(TokenIter & it) {
 ...
 *              | e
 */
ExprPtr AstBasicParser::_buildExprAddSub1(TokenIter & it, ExprPtr pLeft) {
    ExprOp op;
    AST_REC_LOCATION1
    if (it->isPunc<'+'>()) {
        op = ExprOp::OPT_PLUS;
...
ExprPtr AstBasicParser::_buildExprAddSub(TokenIter & it) {
 ...
 *             | e
 */
ExprPtr AstBasicParser::_buildExprShift1(TokenIter & it, ExprPtr pLeft) {
    ExprOp op;
    AST_REC_LOCATION1
    if (it->isPunc<'>>'>()) {
...
ExprPtr AstBasicParser::_buildExprShift(TokenIter & it) {
 ...
 *                | e
 */
ExprPtr AstBasicParser::_buildExprRelation1(TokenIter & it, ExprPtr pLeft) {
    ExprOp op;
    AST_REC_LOCATION1
    if (it->isPunc<'>'>()) {
        op = ExprOp::OPT_GT;
...
ExprPtr AstBasicParser::_buildExprRelation(TokenIter & it) {
 ...
 *            | ε
 */
ExprPtr AstBasicParser::_buildExprEqNe1(TokenIter & it, ExprPtr pLeft) {
    ExprOp op;
    AST_REC_LOCATION1
    if (it->isPunc<'=='>()) {
        op = ExprOp::OPT_EQ;
...
ExprPtr AstBasicParser::_buildExprEqNe(TokenIter & it) {
 ...
 *               | ε
 */
ExprPtr AstBasicParser::_buildExprBitAnd1(TokenIter & it, ExprPtr pLeft) {
    AST_REC_LOCATION1
    if (it->isPunc<'&'>()) {
        ++it;
...
ExprPtr AstBasicParser::_buildExprBitAnd(TokenIter & it) {
 ...
 *               | ε
 */
ExprPtr AstBasicParser::_buildExprBitXor1(TokenIter & it, ExprPtr pLeft) {
    AST_REC_LOCATION1
    if (it->isPunc<'^'>()) {
        ++it;
...
ExprPtr AstBasicParser::_buildExprBitXor(TokenIter & it) {
 ...
 *               | ε
 */
ExprPtr AstBasicParser::_buildExprBitOr1(TokenIter & it, ExprPtr pLeft) {
    AST_REC_LOCATION1
    if (it->isPunc<'|'>()) {
        ++it;
...
ExprPtr AstBasicParser::_buildExprBitOr(TokenIter & it) {
 ...
 *               | ε
 */
ExprPtr AstBasicParser::_buildExprLogicAnd1(TokenIter & it, ExprPtr pLeft) {
    AST_REC_LOCATION1
    if (it->isPunc<'&&'>()) {
        ++it;
...
ExprPtr AstBasicParser::_buildExprLogicAnd(TokenIter & it) {
 ...
 *               | ε
 */
ExprPtr AstBasicParser::_buildExprLogicOr1(TokenIter & it, ExprPtr pLeft) {
    AST_REC_LOCATION1
    if (it->isPunc<'||'>()) {
        ++it;
...
ExprPtr AstBasicParser::_buildExprLogicOr(TokenIter & it) {
 ...
 *          | ExprLogicOr
 */
ExprPtr AstBasicParser::_buildExprCond(TokenIter & it) {
 ...
 *
 */
ExprPtr AstBasicParser::_buildExprAssign1(TokenIter & it, ExprPtr pLeft) {
    ExprOp op;
    AST_REC_LOCATION1
    if (it->isPunc<'+='>()) {
...
ExprPtr AstBasicParser::_buildExprAssign(TokenIter & it) {
 ...
 */
/*
ExprPtr AstBasicParser::_buildExprComma1(TokenIter & it, ExprPtr pLeft) {
    AST_REC_LOCATION1
    if (it->isPunc<','>()) {
        ++ it;
 ...
 *              | e
 */
void AstBasicParser::_parseCommaList1(TokenIter & it, std::vector<ExprPtr> & args) {
    if (it->isPunc<','>()) {
        ++it;
        if (it->isEnd()) {
            AST_E(E_UNEXPECTED_EOF);
...
ExprPtr AstBasicParser::_buildExprComma(TokenIter & it) {
    AST_REC_LOCATION1
...
ExprPtr AstBasicParser::buildExpression(TokenIter & it) {
    if (it->isEnd()) {
        AST_E(E_UNEXPECTED_EOF);
        return nullptr;
    } else {
 ...
 *                          | e
 */
void AstBasicParser::parseScopeObjInitEntityList1(TokenIter & it, std::vector<LocalVar::InitEntityPtr> & entities) {
    if (it->isPunc<','>()) {
        ++it;
        if (it->isPunc<'{'>()) {
            // 说明元素是列表，进入递归
...
void AstBasicParser::parseScopeObjInitEntityList(TokenIter & it, std::vector<LocalVar::InitEntityPtr> & entities) {
    assert(it->isPunc<'{'>());
    ++it;
 ...
 *             | e
 */
void AstBasicParser::parseScopeObjDefineStmt1(TokenIter & it, std::vector<LocalVar> & defScopeObjs,
...
void AstBasicParser::parseScopeObjDefineStmt(TokenIter & it, std::vector<LocalVar> & defScopeObjs,
        const LocalVar & modelScopeObj) {
    LocalVar so;
 ...
 *  | Expr
 */
StmtPtr AstBasicParser::buildDefOrPureExprStmt(TokenIter & it) {
 ...
 *       | e
 */
StmtPtr AstBasicParser::buildStmt(TokenIter & it) {
    if (it->isPunc<'{'>()) { // 解析语句块
        ++it;
...
StmtsPtr AstBasicParser::buildStmts(TokenIter & it) {
    std::vector<StmtPtr> stmts;
    while (true) {
