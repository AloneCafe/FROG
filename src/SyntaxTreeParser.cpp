...
/*
     ...
     *             | e
     */
void AstParser::parseGlobalVarDefineStmt1(TokenIter & it, std::vector<LocalVar> & defGlobalVars,
...
void AstParser::parseGlobalVarDefineStmt(TokenIter & it, std::vector<GlobalVar> & defGlobalVars,
        const GlobalVar & gModelVar) {
 ...
 */
StmtPtr AstParser::buildDefGlobalVarStmt(TokenIter & it, FileStructurePtr pFileStu) {
...
void AstParser::parseFuncRetType(TokenIter & it, Function & mf) {
    VarType type;
    _basicPsr.parseDefType(it, type);
    mf.setRetType(type);
}

void AstParser::parseFuncNameAndArgList(TokenIter & it, Function & mf) {
    if (it->isId()) {
        AST_REC_LOCATION1
 ...
 * FuncContent -> { Stmts }
 */
void AstParser::parseFuncContent(TokenIter & it, FunctionStructure & funcStu) {
    if (it->isPunc<'{'>()) {
        ++ it;
 ...
 */
void AstParser::parseFunc(TokenIter & it, FileStructurePtr pFileStu) {
    Function mf;
    // 处理成员函数
...
FileStructurePtr AstParser::parseTop(TokenIter it) {
...
FileStructurePtr AstParser::parse(const std::vector<Token> & tks) {
    FileStructurePtr pFileStu = parseTop(tks.cbegin());
    return pFileStu;
}

const std::vector<Error> & AstParser::getErrList() {
    return _aed.getErrList();
}

bool AstParser::hasErr() const {
    return _aed.hasErr();
}
